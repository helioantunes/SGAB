...
let senhaEle = document.querySelector("#senha2");
senhaEle.addEventListener("change", () => {
    let regexEle = /^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$ %^&*-]).{8,}$/;
    let smallEle = document.querySelector("#senha2 ~ small");
    let labelEle = document.querySelector("label[for=senha2]");
    let inputObrigatorioEle = document.querySelector("label[for=senha2] .input-obrigatorio");

    validacaoView(senhaEle, regexEle, smallEle, labelEle, inputObrigatorioEle);
});

let senha2Ele = document.querySelector("#senha3");
